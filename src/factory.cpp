    ...
    std::map<const PackageSender*, NodeColor>& node_colors)
{
    ...
    for (const auto& [receiver, prob] : prefs) {
        ...
        if (receiver->get_receiver_type() == ReceiverType::STOREHOUSE) {
            continue; // ta gałąź OK, sprawdzamy resztę
        }
    
        auto worker = dynamic_cast<const Worker*>(receiver);
        if (!worker) {
            return false; // receiver nie-worker i nie-storehouse
        }
    
        if (!has_reachable_storehouse(worker, node_colors)) {
            return false; // JEDNA ślepa gałąź = fail
        }
    }
...
bool Factory::is_consistent() const {
    ...
    for (const auto& ramp : ramps_) {
        if (ramp.receiver_preferences().get_preferences().empty()) {
            return false;
        }
    }

    for (const auto& worker : workers_) {
        if (worker.receiver_preferences().get_preferences().empty()) {
            return false;
        }
    }
